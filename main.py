...
def _parse_qs(path):
    """パスの "?a=1&b=2" 部分を {"a":"1","b":"2"} の辞書にして返す"""
    qs = {}
    if '?' in path:
...
def _qint(qs, key, default):
    """qs[key] を整数に変換して返す。無い／変換できないときは default"""
    try:
        return int(qs[key])
    except:
        return default
...
async def handle_http(reader, writer):
    ...
    try:
        ...
        if ('generate_204' in path or 'gen_204'     in path or
            ...
            qs   = _parse_qs(path)
            r2   = max(0, min(255, _qint(qs, 'r', 0)))
            g2   = max(0, min(255, _qint(qs, 'g', 0)))
            b2   = max(0, min(255, _qint(qs, 'b', 0)))
            save = 'save' in qs
            set_eyes(r2, g2, b2)
            if save:
            ...
            qs = _parse_qs(path)
            v  = max(0, min(100, _qint(qs, 'v', VOLUME)))
            VOLUME = min(VOLUME_LEVELS, key=lambda x: abs(x - v))
            save_settings()
            ...
            qs    = _parse_qs(path)
            angle = max(0, min(180, _qint(qs, 'angle', 90)))
            if not busy:
                last_interaction = time.time()
            ...
            qs   = _parse_qs(path)
            freq = max(100, min(5000, _qint(qs, 'freq', 440)))
            buzzer.freq(freq)
            buzzer.duty_u16(_buzz_duty())
            writer.write(b'HTTP/1.1 200 OK\r\nContent-Length: 0\r\nConnection: close\r\n\r\n')
